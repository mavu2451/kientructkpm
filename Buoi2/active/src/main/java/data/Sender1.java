		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					Destination destination = (Destination) ctx.lookup("dynamicQueues/" + ten);
					...
					try(Jedis jedis = pool.getResource()){
						jedis.publish(ten, textField.getText().toString());
			...
			public void actionPerformed(ActionEvent e) {
				try(Jedis jedis = pool.getResource()) {
					jedis.subscribe(new JedisPubSub() {
						public void onMessage(String chanel, String txt) {
								System.out.println("Chanel " + chanel + " has sent a message: " + txt);
								textArea.setText("Xin chao");
...
//					Destination destination = (Destination) ctx.lookup("dynamicQueues/" + ten1);
//					Connection con = factory.createConnection("admin", "admin");
//					con.start();
