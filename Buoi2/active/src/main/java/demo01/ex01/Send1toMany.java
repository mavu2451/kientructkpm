		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					String name1= "thanthidet11";
					String name2= "thanthidet12";
					String name3= "thanthidet13";
					...
					Destination destination = (Destination) ctx.lookup("dynamicQueues/" + name1);
					Destination destination1 = (Destination) ctx.lookup("dynamicQueues/" + name2);
					Destination destination2 = (Destination) ctx.lookup("dynamicQueues/" + name3);
					...
					producer1.send(msg);
					producer2.send(msg);
					String ten = "Thân Thị Đẹt";
					Person p = new Person(1001, ten, new Date());
					String xml = new XMLConvert<Person>(p).object2XML(p);
					
					//Cau lenh redis
					try(Jedis jedis = pool.getResource()){
						jedis.set(ten, textField.getText().toString());
					}catch (Exception e2) {
						// TODO: handle exception
						e2.printStackTrace();
						System.out.println("Loi khong ket noi duoc voi Redis");
					}
					//
					session.close();
					con.close();
